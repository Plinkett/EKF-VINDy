...
def train_vindy(vindy_layer: nn.Module, z_train: torch.Tensor, dzdt_train: torch.Tensor, n_epochs = 1000, lr = 1e-3, 
                huber_weight = 1.0, kl_weight = 1e-3):
    ...
    for epoch in range(n_epochs):
        ...
        # Forward pass
        dzdt_pred = vindy_layer(z_train)
        
        # Huber loss (or MSE if you want...)
        huber = F.huber_loss(dzdt_pred, dzdt_train, delta=1.0, reduction='mean')
        
        # KL divergence
        kl = vindy_layer.big_xi_distribution.kl_divergence(vindy_layer.laplace_prior)
        kl = kl.sum()
        ...
        if epoch % 50 == 0:
            print(f"[{epoch}] loss={loss.item():.4e}   mse={huber.item():.4e}   L1={kl.item():.4e}")
